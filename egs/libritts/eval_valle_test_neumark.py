...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Number of test samples to evaluate (default: None for full test set)",
    )
    parser.add_argument(
        "--save-audio-count",
        type=int,
        default=50,
        help="Maximum number of audio pairs to save as WAV files (default: 50)",
    )
    ...
    print(f"Evaluating Samples: {'Full Test Set' if args.num_samples is None or args.num_samples <= 0 else args.num_samples}")
    print(f"Max Saved Audios:   {args.save_audio_count}")
    print("--------------------------------------------------------------------------------")
    ...
    for c in cuts_iter:
        if 2.0 <= c.duration <= 10.0:
            test_cuts.append(c)
            if args.num_samples is not None and args.num_samples > 0 and len(test_cuts) >= args.num_samples:
                break
    ...
    for i, cut in enumerate(test_cuts):
        ...
        run_worker_for_sample(
            ...
            save_audio=(idx <= args.save_audio_count),
        )
