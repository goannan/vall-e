...
class NeuMarkValleControlTrainer:
    """
    Control Group Trainer:
    - Uses synthetic VALL-E tokens as input dataset.
    - Reference audio is 'recon' (SpeechTokenizer clean synthesis from synthetic tokens).
    - Loss functions: NeuMark multi-scale Mel loss, Latent Cosine loss, GAN on recon, VAD, Decoding.
    - Loss weights: Classic NeuMark weights (mel=0.25, cos=2.0, adv=1.0, dec=10.0, vad=1.0).
    """
    ...
    def __init__(self, cfg: dict):
        ...
        # Loss weights (NeuMark configuration)
        self.mel_loss_lambda = cfg.get("mel_loss_lambda", 0.25)
        ...
        # Experimental TTS-Native losses (kept 0 in control group)
        self.utmos_loss_lambda = cfg.get("utmos_loss_lambda", 0.0)
        self.sim_loss_lambda = cfg.get("sim_loss_lambda", 0.0)
        self.asr_loss_lambda = cfg.get("asr_loss_lambda", 0.0)
        ...
        # Setup Logging
        base_results = Path(cfg.get("results_folder", "exp/tts_valle_neumark_control"))
        ...
        # Multi-scale Mel Loss Module
        self.mel_loss_fn = MultiScaleMelLoss(
            sample_rate=self.sample_rate,
            n_fft=cfg.get("n_fft", 1024),
            hop_size=cfg.get("hop_size", 240),
            win_size=cfg.get("win_size", 1024),
            num_mels=cfg.get("num_mels", 80),
            fmin=cfg.get("fmin", 0.0),
            fmax=cfg.get("fmax", 8000.0),
            lambdas=cfg.get("multi_scale_mel_loss_lambdas", [5.0, 1.0, 1.0, 1.0]),
        )

        # Validation Diagnostic Models (UTMOS, WavLM-SIM, Wav2Vec2-ASR)
        print("[Init] Initializing Diagnostic Evaluation Models (UTMOS, WavLM-SIM, Wav2Vec2-ASR)...")
        ...
        # ---------------------------------------------------------
        # Dataset & Dataloader (VALL-E Synthesized Tokens)
        # ---------------------------------------------------------
        print("[Init] Loading Synthetic Tokens Manifest (Control Group)...")
        train_manifest = str(SCRIPT_DIR / cfg.get("train_manifest"))
        valid_manifest = str(SCRIPT_DIR / cfg.get("valid_manifest"))
        max_dur = cfg.get("max_duration", 10.5)
    ...
    def validate(self, step: int):
        ...
        with torch.no_grad():
            ...
            for batch in self.valid_dl:
                ...
                # 1. Clean Synthesis from Synthetic Tokens (Recon)
                ...
                for cat, name, detail, atk_fn in val_attacks:
                    ...
                    # 2. Clean Unwatermarked Recon Audio (+ Attack) -> Evaluates True Negatives (TN)
                    try:
    ...
    def train(self):
        print(f"[Training] Starting NeuMark Control Training for {self.epochs} epochs...")
        self.generator.train()
        ensure_frozen_model_train_mode(self.generator)
        for m in self.generator.modules():
            if isinstance(m, (torch.nn.BatchNorm1d, torch.nn.BatchNorm2d, torch.nn.LayerNorm)):
        ...
        for epoch in range(self.epochs):
            for i, batch in enumerate(tqdm(self.dl, desc=f"Epoch {epoch}", disable=not self.is_main, ncols=100)):
                ...
                # -------------------------------------------------------------
                # 1. Decode Synthetic Codes & Embed Watermark
                # -------------------------------------------------------------
                ...
                # Reference audio: Clean synthesis from synthetic tokens (recon)
                # Watermarked audio: Watermarked synthesis from synthetic tokens (wm)
                recon_audio = unwrapped_gen.decoder(z_q)
                wm_audio = unwrapped_gen.decoder(z_wm)

                # Align temporal dimensions
                min_len = min(recon_audio.shape[-1], wm_audio.shape[-1])
                recon_audio_aligned = recon_audio[..., :min_len]
                wm_audio_aligned = wm_audio[..., :min_len]

                # -------------------------------------------------------------
                # 2. Compute NeuMark Losses (Ref = recon_audio)
                # -------------------------------------------------------------
                # A. Multi-scale Mel Spectrogram Loss (recon vs wm)
                loss_mel = self.mel_loss_fn(recon_audio_aligned, wm_audio_aligned) * self.mel_loss_lambda

                # B. Latent Space Cosine Loss (z_wm vs z_q)
                loss_cos = latent_cosine_loss(z_wm, z_q) * self.cos_loss_lambda

                # C. GAN Adversarial & Feature Matching (Real = recon_audio, Fake = wm_audio)
                ...
                # D. Watermark Decoding & Detection Loss
                augmented_audio, vad_labels, attack_name = apply_train_augmentation(
                    wm_audio, sample_rate=self.sample_rate, orig_audio=recon_audio
                )
                ...
                # Negative sample supervision (Unwatermarked synthetic recon audio)
                ...
                # -------------------------------------------------------------
                # 4. Train Discriminators (Real = recon_audio, Fake = wm_audio)
                # -------------------------------------------------------------
        ...
        print(f"[Done] Control training complete after {steps} steps.")
...
def main():
    ...
    parser.add_argument("--config", default=str(SCRIPT_DIR / "config_tts_valle_control.json"), help="Config path")
    args = parser.parse_args()

    with open(args.config) as f:
        cfg = json.load(f)

    trainer = NeuMarkValleControlTrainer(cfg)
    trainer.train()
