#!/usr/bin/env python3
"""
Optimized High-Throughput Full-Dataset Evaluation of PESQ (WB 16kHz) and STOI for 5 Watermark Models:
...
Evaluates ALL 2,078 cuts on LibriTTS (test-clean) and ALL 1,088 cuts on SeedTTS.
Supports checkpoint caching so completed models are never re-evaluated.
"""
...
RESULTS_JSON = SCRIPT_DIR / "full_dataset_pesq_stoi_results.json"
...
class WatermarkRunner:
    ...
    def embed_audio(self, clean_audio: torch.Tensor, msg_np: np.ndarray, msg_tensor: torch.Tensor) -> torch.Tensor:
        if self.model_name in ["proposed", "neumark"]:
            with torch.no_grad():
            ...
            return torch.from_numpy(sig_wm).float().unsqueeze(0).unsqueeze(0).to(self.device)
...
def run_model_on_dataset(runner: WatermarkRunner, dataset: CleanAudioDataset, num_workers: int = 24) -> Tuple[float, float]:
    ...
    with ProcessPoolExecutor(max_workers=num_workers) as executor:
        for wav_1d, _ in tqdm(loader, desc=f"Embedding [{runner.model_name}]", ncols=90):
            ...
            wm_tensor = runner.embed_audio(clean_tensor, msg_np, msg_tensor)
            
            min_len = min(clean_tensor.shape[-1], wm_tensor.shape[-1])
            ref_np = clean_tensor[0, 0, :min_len].detach().cpu().numpy()
...
def sync_summary_files(full_results: dict):
    ...
    logging.info(f"Saved full PESQ/STOI results to {RESULTS_JSON}")
...
def main():
    ...
    logging.info("=" * 85)
    logging.info(f" Starting Concurrent FULL-DATASET PESQ & STOI Evaluation on device: {device}")
    logging.info("=" * 85)
    ...
    models = ["proposed", "audioseal", "neumark", "traceablespeech", "wavmark"]
